...
export function Navbar({className, items, onSelect}: NavBarProps) {
    ...
    return (
        <>
            ...
            <nav
                className={`fixed top-0 z-10 flex h-screen w-72 max-lg:w-72 ${sidebar ? "lg:w-72" : "lg:w-24"} transform-gpu flex-col rounded-r-2xl bg-background drop-shadow-md duration-500 ${
            ...
            >
                ...
                <NavItems className={"mt-20"} items={items} onSelect={onSelect} isFull={sidebar}/>
            </nav>
        </>
    );
}
...
export function NavMenuItem({name, isFull, Icon, onClick}: NavMenuItemProps) {
    return (
        <div
            className={`z-10 flex h-16 cursor-pointer overflow-clip flex-row justify-evenly flex-nowrap bg-transparent ring-transparent hover:bg-transparent`}
            onClick={onClick}
        >
            <div className={`h-16 w-1 shrink-0`}/>
            <div className={'w-4 shrink-0'}/>
            <div className={`flex min-w-0 grow shrink flex-shrink h-16 flex-row`}>
                <Icon className={"h-16 w-16 shrink-0 p-3"}/>
                <span
                    className={`my-auto min-w-0 justify-center w-fit whitespace-nowrap ${isFull ? "lg:visible lg:w-fit" : "lg:invisible lg:w-0"} delay-500 transition overflow-clip ease-in-out`}>{name}</span>
            </div>
        </div>
    );
}
